Creativecoin Core version 0.13.2 is now available from:
...
ECDSA signatures inside Creativecoin transactions now use validation using
...
Previous versions of Creativecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Creativecoin Core was to raise the default minimum
relay fee.

Creativecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Creativecoin Core 0.13.2 also introduces new default policy limits on the length and
...
Creativecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Creativecoin Core will relay transactions with
...
entirely in Creativecoin Core 0.13, and it is at this time undecided whether the
...
Creativecoin Core automatically creates a hidden service to listen on, without
manual configuration. Creativecoin Core will also use Tor automatically to connect
...
Creativecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Creativecoin
Core to use floating fees. By default, Creativecoin Core will use floating
fees.
...
(default: `0.0002` CREA/kB).

At all times, Creativecoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CREA.
Furthermore, Creativecoin Core will never create transactions paying less than
...
Various code modernizations have been done. The Creativecoin Core code base has
...
for a new opcode in the Creativecoin scripting system that in combination with
...
Creativecoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Creativecoin Core.
...
The two phases are distinct in the Creativecoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Creativecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Creativecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Creativecoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Creativecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Creativecoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Creativecoin's current ledger (called the UTXO set),
  ...
  note that Creativecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Creativecoin users to individually opt-in to almost any change in the Creativecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Creativecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Creativecoin
client to Creativecoin Core 0.13.2 to signal for segwit support. When a
...
Since Creativecoin Core 0.10.0, nodes have defaulted to only relaying and
