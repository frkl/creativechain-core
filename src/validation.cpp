...
#if defined(NDEBUG)
# error "Creativecoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Creativecoin Signed Message:\n";
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block, block.nBits, consensusParams))
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.vtx.empty() || block.vtx.size() > MAX_BLOCK_BASE_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_BLOCK_BASE_SIZE) {
        error("bad-blk-length size limits failed");
    ...
    for (unsigned int i = 1; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinBase()) {
            error("bad-cd-multiple more than one coinbase");
    ...
    if (nSigOps * WITNESS_SCALE_FACTOR > MAX_BLOCK_SIGOPS_COST) {
        error("bad-blk-sigops out-of-bounds SigOpCount");
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Creativecoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
